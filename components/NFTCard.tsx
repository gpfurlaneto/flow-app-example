...
export default function NFTCard({ nft, price, onClickBuy }: NFTCardProps) {
  ...
  const url = `https://gateway.pinata.cloud/ipfs/${nft.view.replace(
    "ipfs://",
    ""
  )}`;
    ...
    <div className="max-w-sm rounded overflow-hidden shadow-lg border border-solid">
      ...
      <div className="px-6 py-4">
        ...
        <p className="text-base">
          <span className="font-bold text-sm mb-2">Owner: </span>
          {formatAddress(nft.owner as string)}
        </p>
        {price && (
      ...
      <div className="px-6 pb-2 flex items-end">
        {currentUser?.addr !== nft.owner && (
          <Button className="ml-auto w-full" onClick={onClickBuy}>
            Buy
          </Button>
        )}
      </div>
