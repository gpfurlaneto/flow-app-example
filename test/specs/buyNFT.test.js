...
import { buyItem, getFlowBalance, listNFT, mintFlowToken, mintNFT, setupStorefrontV2 } from '../utils'
...
describe.only("Buy NFT", () => {
  ...
  test("Buy NFT to alice account", async () => {
    ...
    const resultListing = await shallPass(listNFT(EMULATOR_ACCOUNT, "0", "1.0", expiry.getTime().toString()))
    const { data: { listingResourceID } } = resultListing[0].events[0]

    const aliceAccount = await getAccountAddress("alice")
    await shallPass(mintFlowToken(EMULATOR_ACCOUNT, aliceAccount, "1.0"))

    const resultBuy = await shallPass(buyItem(aliceAccount, EMULATOR_ACCOUNT, listingResourceID))

    const [balanceAfterSell] = await getFlowBalance(EMULATOR_ACCOUNT)
    expect(balanceAfterSell).toBe('1000000000.99600000')
    
    const listinCompletedEvent = resultBuy[0].events.find(event => event.type === 'A.f8d6e0586b0a20c7.NFTStorefrontV2.ListingCompleted')
